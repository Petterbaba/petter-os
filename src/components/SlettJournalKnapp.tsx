...
import { slettJournalAction } from "@/app/journal/actions";
...
// Egen liten form per innførsel: native confirm() som angrevern før den
// endelige slettingen (ingen egen dialogkomponent i v0.1).
export function SlettJournalKnapp({ id }: { id: string }) {
  ...
  return (
    <form
      action={handling}
      onSubmit={(hendelse) => {
        if (!window.confirm("Slette innførselen? Dette kan ikke angres.")) {
